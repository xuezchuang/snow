...
//! Constructor
COpenGLParallaxMapRendererOld::COpenGLParallaxMapRendererOld(video::COpenGLDriverOld* driver,
													   s32& outMaterialTypeNr, E_MATERIAL_TYPE baseMaterial)
	: COpenGLShaderMaterialRendererOld(driver, 0, baseMaterial), CompiledShaders(true)
{

#ifdef _DEBUG
	setDebugName("COpenGLParallaxMapRendererOld");
#endif
	...
	if (renderer)
	{
		...
		video::COpenGLParallaxMapRendererOld* nmr = reinterpret_cast<video::COpenGLParallaxMapRendererOld*>(renderer);
...
//! Destructor
COpenGLParallaxMapRendererOld::~COpenGLParallaxMapRendererOld()
{
	if (CallBack == this)
		CallBack = 0;
...
void COpenGLParallaxMapRendererOld::OnSetMaterial(const video::SMaterial& material,
											   const video::SMaterial& lastMaterial,
											   bool resetAllRenderstates, video::IMaterialRendererServices* services)
{
	COpenGLShaderMaterialRendererOld::OnSetMaterial(material, lastMaterial,
...
s32 COpenGLParallaxMapRendererOld::getRenderCapability() const
{
	if (Driver->queryFeature(video::EVDF_ARB_FRAGMENT_PROGRAM_1) &&
		Driver->queryFeature(video::EVDF_ARB_VERTEX_PROGRAM_1))
		return 0;
...
void COpenGLParallaxMapRendererOld::OnSetConstants(IMaterialRendererServices* services, s32 userData)
{

...
void pGraphics::createBuffers()
{
	...
	for (size_t i = 0; i < Graphics.renderers.size(); i++) urenderers[Graphics.renderers[i]->mesh.ID].renderers.push_back(Graphics.renderers[i]);
...
void pGraphics::draw(GLuint &shader_program, glm::vec2 screen_size)
{
	...
	for (const auto &rend : urenderers)
	{
		...
		for (size_t m = 0; m < urenderers[rend.first].renderers.size(); m++)
		{

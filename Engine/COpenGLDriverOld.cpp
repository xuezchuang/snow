...
#include "COpenGLCacheHandlerOld.h"
#include "COpenGLMaterialRendererOld.h"
#include "COpenGLShaderMaterialRendererOld.h"
#include "COpenGLSLMaterialRendererOld.h"
#include "COpenGLNormalMapRendererOld.h"
#include "COpenGLParallaxMapRendererOld.h"
...
// Statics variables
const u16 COpenGLDriverOld::Quad2DIndices[4] = { 0, 1, 2, 3 };

#if defined(_IRR_COMPILE_WITH_WINDOWS_DEVICE_) || defined(_IRR_COMPILE_WITH_X11_DEVICE_) || defined(_IRR_COMPILE_WITH_OSX_DEVICE_)
COpenGLDriverOld::COpenGLDriverOld(const SIrrlichtCreationParameters& params, io::IFileSystem* io, IContextManager* contextManager)
...
{
#ifdef _DEBUG
	setDebugName("COpenGLDriverOld");
#endif
}
#endif

#ifdef _IRR_COMPILE_WITH_SDL_DEVICE_
COpenGLDriverOld::COpenGLDriverOld(const SIrrlichtCreationParameters& params, io::IFileSystem* io, CIrrDeviceSDL* device)
...
bool COpenGLDriverOld::initDriver()
{
...
//! destructor
COpenGLDriverOld::~COpenGLDriverOld()
{
...
bool COpenGLDriverOld::genericDriverInit()
{
	if (ContextManager)
		ContextManager->grab();
	...
	// reset cache handler
	delete CacheHandler;
	CacheHandler = new COpenGLCacheHandlerOld(this);
...
void COpenGLDriverOld::createMaterialRenderers()
{
	// create OpenGL material renderers

	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_SOLID_OLD(this));
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_SOLID_2_LAYER_OLD(this));

	// add the same renderer for all lightmap types
	COpenGLMaterialRenderer_LIGHTMAP_OLD* lmr = new COpenGLMaterialRenderer_LIGHTMAP_OLD(this);
	...
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_DETAIL_MAP_OLD(this));
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_SPHERE_MAP_OLD(this));
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_REFLECTION_2_LAYER_OLD(this));
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_TRANSPARENT_ADD_COLOR_OLD(this));
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_TRANSPARENT_ALPHA_CHANNEL_OLD(this));
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_TRANSPARENT_ALPHA_CHANNEL_REF_OLD(this));
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_TRANSPARENT_VERTEX_ALPHA_OLD(this));
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_TRANSPARENT_REFLECTION_2_LAYER_OLD(this));

	// add normal map renderers
	s32 tmp = 0;
	video::IMaterialRenderer* renderer = 0;
	renderer = new COpenGLNormalMapRendererOld(this, tmp, EMT_SOLID);
	renderer->drop();
	renderer = new COpenGLNormalMapRendererOld(this, tmp, EMT_TRANSPARENT_ADD_COLOR);
	renderer->drop();
	renderer = new COpenGLNormalMapRendererOld(this, tmp, EMT_TRANSPARENT_VERTEX_ALPHA);
	renderer->drop();

	// add parallax map renderers
	renderer = new COpenGLParallaxMapRendererOld(this, tmp, EMT_SOLID);
	renderer->drop();
	renderer = new COpenGLParallaxMapRendererOld(this, tmp, EMT_TRANSPARENT_ADD_COLOR);
	renderer->drop();
	renderer = new COpenGLParallaxMapRendererOld(this, tmp, EMT_TRANSPARENT_VERTEX_ALPHA);
	renderer->drop();

	// add basic 1 texture blending
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_ONETEXTURE_BLEND_OLD(this));
}

bool COpenGLDriverOld::beginScene(u16 clearFlag, SColor clearColor, f32 clearDepth, u8 clearStencil, const SExposedVideoData& videoData, core::rect<s32>* sourceRect)
{
...
bool COpenGLDriverOld::endScene()
{
...
const core::matrix4& COpenGLDriverOld::getTransform(E_TRANSFORMATION_STATE state) const
{
...
//! sets transformation
void COpenGLDriverOld::setTransform(E_TRANSFORMATION_STATE state, const core::matrix4& mat)
{
	Matrices[state] = mat;
...
bool COpenGLDriverOld::updateVertexHardwareBuffer(SHWBufferLink_opengl* HWBuffer)
{
	if (!HWBuffer)
		return false;
...
bool COpenGLDriverOld::updateIndexHardwareBuffer(SHWBufferLink_opengl* HWBuffer)
{
	if (!HWBuffer)
		return false;
...
bool COpenGLDriverOld::updateHardwareBuffer(SHWBufferLink* HWBuffer)
{
	if (!HWBuffer)
		return false;
...
COpenGLDriverOld::SHWBufferLink* COpenGLDriverOld::createHardwareBuffer(const scene::IMeshBuffer* mb)
{
#if defined(GL_ARB_vertex_buffer_object)
...
void COpenGLDriverOld::deleteHardwareBuffer(SHWBufferLink* _HWBuffer)
{
	if (!_HWBuffer)
		return;
...
void COpenGLDriverOld::drawHardwareBuffer(SHWBufferLink* _HWBuffer)
{
	if (!_HWBuffer)
		return;
...
void COpenGLDriverOld::addOcclusionQuery(scene::ISceneNode* node,
...
void COpenGLDriverOld::removeOcclusionQuery(scene::ISceneNode* node)
{
	const s32 index = OcclusionQueries.linear_search(SOccQuery(node));
	if (index != -1)
	{
...
void COpenGLDriverOld::runOcclusionQuery(scene::ISceneNode* node, bool visible)
{
	if (!node)
		return;
...
void COpenGLDriverOld::updateOcclusionQuery(scene::ISceneNode* node, bool block)
{
	const s32 index = OcclusionQueries.linear_search(SOccQuery(node));
	if (index != -1)
	{
...
u32 COpenGLDriverOld::getOcclusionQueryResult(scene::ISceneNode* node) const
{
	const s32 index = OcclusionQueries.linear_search(SOccQuery(node));
	if (index != -1)
...
IRenderTarget* COpenGLDriverOld::addRenderTarget()
{
	COpenGLRenderTargetOld* renderTarget = new COpenGLRenderTargetOld(this);
...
void COpenGLDriverOld::drawVertexPrimitiveList(const void* vertices, u32 vertexCount,
...
void COpenGLDriverOld::getColorBuffer(const void* vertices, u32 vertexCount, E_VERTEX_TYPE vType)
{
...
void COpenGLDriverOld::renderArray(const void* indexList, u32 primitiveCount,
...
void COpenGLDriverOld::draw2DVertexPrimitiveList(const void* vertices, u32 vertexCount,
											  const void* indexList, u32 primitiveCount,
											  E_VERTEX_TYPE vType, scene::E_PRIMITIVE_TYPE pType, E_INDEX_TYPE iType)
{
	if (!primitiveCount || !vertexCount)
...
void COpenGLDriverOld::draw2DImage(const video::ITexture* texture, const core::position2d<s32>& destPos,
...
void COpenGLDriverOld::draw2DImage(const video::ITexture* texture, const core::rect<s32>& destRect,
...
void COpenGLDriverOld::draw2DImage(const video::ITexture* texture, u32 layer, bool flip)
{
...
//! texture if desired.
void COpenGLDriverOld::draw2DImageBatch(const video::ITexture* texture,
...
//! draw a 2d rectangle
void COpenGLDriverOld::draw2DRectangle(SColor color, const core::rect<s32>& position,
...
void COpenGLDriverOld::draw2DRectangle(const core::rect<s32>& position,
...
//! Draws a 2d line.
void COpenGLDriverOld::draw2DLine(const core::position2d<s32>& start,
...
//! Draws a pixel
void COpenGLDriverOld::drawPixel(u32 x, u32 y, const SColor& color)
{
	const core::dimension2d<u32>& renderTargetSize = getCurrentRenderTargetSize();
...
bool COpenGLDriverOld::disableTextures(u32 fromStage)
{
	bool result = true;
...
inline void COpenGLDriverOld::getGLMatrix(GLfloat gl_matrix[16], const core::matrix4& m)
{
...
inline void COpenGLDriverOld::getGLTextureMatrix(GLfloat* o, const core::matrix4& m)
{
	o[0] = m[0];
	o[1] = m[1];
	o[2] = 0.f;
...
ITexture* COpenGLDriverOld::createDeviceDependentTexture(const io::path& name, IImage* image)
{
	core::array<IImage*> imageArray(1);
	imageArray.push_back(image);

	COpenGLTextureOld* texture = new COpenGLTextureOld(name, imageArray, ETT_2D, this);

	return texture;
}

ITexture* COpenGLDriverOld::createDeviceDependentTextureCubemap(const io::path& name, const core::array<IImage*>& image)
{
	COpenGLTextureOld* texture = new COpenGLTextureOld(name, image, ETT_CUBEMAP, this);

	return texture;
}

void COpenGLDriverOld::disableFeature(E_VIDEO_DRIVER_FEATURE feature, bool flag)
{
...
void COpenGLDriverOld::setMaterial(const SMaterial& material)
{
	Material = material;
...
bool COpenGLDriverOld::testGLError(int code)
{
#ifdef _DEBUG
	GLenum g = glGetError();
	switch (g)
...
void COpenGLDriverOld::setRenderStates3DMode()
{
...
GLint COpenGLDriverOld::getTextureWrapMode(const u8 clamp)
{
...
void COpenGLDriverOld::setBasicRenderStates(const SMaterial& material, const SMaterial& lastmaterial,
...
void COpenGLDriverOld::setTextureRenderStates(const SMaterial& material, bool resetAllRenderstates)
{
	...
	for (s32 i = Feature.MaxTextureUnits - 1; i >= 0; --i)
	{
		...
		const COpenGLTextureOld* tmpTexture = CacheHandler->getTextureCache().get(i);
		...
		if (tmpTexture)
		{
			...
			COpenGLTextureOld::SStatesCache& statesCache = tmpTexture->getStatesCache();
...
void COpenGLDriverOld::enableMaterial2D(bool enable)
{
	if (!enable)
...
//! sets the needed renderstates
void COpenGLDriverOld::setRenderStates2DMode(bool alpha, bool texture, bool alphaChannel)
{
	// 2d methods uses fixed pipeline
	if (FixedPipelineState == COpenGLDriverOld::EOFPS_DISABLE)
		FixedPipelineState = COpenGLDriverOld::EOFPS_DISABLE_TO_ENABLE;
	else
		FixedPipelineState = COpenGLDriverOld::EOFPS_ENABLE;
...
const wchar_t* COpenGLDriverOld::getName() const
{
	return Name.c_str();
}
...
void COpenGLDriverOld::deleteAllDynamicLights()
{
...
s32 COpenGLDriverOld::addDynamicLight(const SLight& light)
{
...
void COpenGLDriverOld::assignHardwareLight(u32 lightIndex)
{
...
void COpenGLDriverOld::turnLightOn(s32 lightIndex, bool turnOn)
{
...
u32 COpenGLDriverOld::getMaximalDynamicLightAmount() const
{
	return MaxLights;
}
...
void COpenGLDriverOld::setAmbientLight(const SColorf& color)
{
...
// method just a bit.
void COpenGLDriverOld::setViewPort(const core::rect<s32>& area)
{
...
void COpenGLDriverOld::drawStencilShadowVolume(const core::array<core::vector3df>& triangles, bool zfail, u32 debugDataVisible)
{
...
void COpenGLDriverOld::drawStencilShadow(bool clearStencilBuffer, video::SColor leftUpEdge,
...
//! Sets the fog mode.
void COpenGLDriverOld::setFog(SColor c, E_FOG_TYPE fogType, f32 start,
...
//! Draws a 3d box.
void COpenGLDriverOld::draw3DBox(const core::aabbox3d<f32>& box, SColor color)
{
...
//! Draws a 3d line.
void COpenGLDriverOld::draw3DLine(const core::vector3df& start,
...
void COpenGLDriverOld::removeTexture(ITexture* texture)
{
...
bool COpenGLDriverOld::queryTextureFormat(ECOLOR_FORMAT format) const
{
...
void COpenGLDriverOld::OnResize(const core::dimension2d<u32>& size)
{
...
E_DRIVER_TYPE COpenGLDriverOld::getDriverType() const
{
	return EDT_OPENGL;
}
...
ECOLOR_FORMAT COpenGLDriverOld::getColorFormat() const
{
	return ColorFormat;
}
...
s32 COpenGLDriverOld::getVertexShaderConstantID(const c8* name)
{
	return getPixelShaderConstantID(name);
}

//! Get a pixel shader constant index.
s32 COpenGLDriverOld::getPixelShaderConstantID(const c8* name)
{
...
void COpenGLDriverOld::setVertexShaderConstant(const f32* data, s32 startRegister, s32 constantAmount)
{
...
void COpenGLDriverOld::setPixelShaderConstant(const f32* data, s32 startRegister, s32 constantAmount)
{
...
bool COpenGLDriverOld::setVertexShaderConstant(s32 index, const f32* floats, int count)
{
...
bool COpenGLDriverOld::setVertexShaderConstant(s32 index, const s32* ints, int count)
{
	return setPixelShaderConstant(index, ints, count);
}

//! Sets a constant for the pixel shader based on an index.
bool COpenGLDriverOld::setPixelShaderConstant(s32 index, const f32* floats, int count)
{
...
//! Int interface for the above.
bool COpenGLDriverOld::setPixelShaderConstant(s32 index, const s32* ints, int count)
{
	os::Printer::log("Error: Please call services->setPixelShaderConstant(), not VideoDriver->setPixelShaderConstant().");
	return false;
}
...
s32 COpenGLDriverOld::addShaderMaterial(const c8* vertexShaderProgram,
									 const c8* pixelShaderProgram,
									 IShaderConstantSetCallBack* callback,
									 E_MATERIAL_TYPE baseMaterial, s32 userData)
{
	s32 nr = -1;
	COpenGLShaderMaterialRendererOld* r = new COpenGLShaderMaterialRendererOld(
...
s32 COpenGLDriverOld::addHighLevelShaderMaterial(
	...
	s32 userData)
{
	...
	COpenGLSLMaterialRendererOld* r = new COpenGLSLMaterialRendererOld(
		this, nr,
...
IVideoDriver* COpenGLDriverOld::getVideoDriver()
{
	return this;
}


ITexture* COpenGLDriverOld::addRenderTargetTexture(const core::dimension2d<u32>& size,
												const io::path& name, const ECOLOR_FORMAT format)
{
	...
	COpenGLTextureOld* renderTargetTexture = new COpenGLTextureOld(name, destSize, ETT_2D, format, this);
...
ITexture* COpenGLDriverOld::addRenderTargetTextureCubemap(const irr::u32 sideLen, const io::path& name, const ECOLOR_FORMAT format)
{
	//disable mip-mapping
	bool generateMipLevels = getTextureCreationFlag(ETCF_CREATE_MIP_MAPS);
	setTextureCreationFlag(ETCF_CREATE_MIP_MAPS, false);
	...
	COpenGLTextureOld* renderTargetTexture = new COpenGLTextureOld(name, destSize, ETT_CUBEMAP, format, this);
	addTexture(renderTargetTexture);
	renderTargetTexture->drop();
...
//! call.
u32 COpenGLDriverOld::getMaximalPrimitiveCount() const
{
	return 0x7fffffff;
}

bool COpenGLDriverOld::setRenderTargetEx(IRenderTarget* target, u16 clearFlag, SColor clearColor, f32 clearDepth, u8 clearStencil)
{
	...
	if (target)
	{
		COpenGLRenderTargetOld* renderTarget = static_cast<COpenGLRenderTargetOld*>(target);
	...
	else
	{
		...
		{
			COpenGLRenderTargetOld* prevRenderTarget = static_cast<COpenGLRenderTargetOld*>(CurrentRenderTarget);
			COpenGLTextureOld* renderTargetTexture = static_cast<COpenGLTextureOld*>(prevRenderTarget->getTexture());

			if (renderTargetTexture)
			{
				const COpenGLTextureOld* prevTexture = CacheHandler->getTextureCache()[0];
...
void COpenGLDriverOld::clearBuffers(u16 flag, SColor color, f32 depth, u8 stencil)
{
	GLbitfield mask = 0;
	u8 colorMask = 0;
...
IImage* COpenGLDriverOld::createScreenShot(video::ECOLOR_FORMAT format, video::E_RENDER_TARGET target)
{
...
bool COpenGLDriverOld::setClipPlane(u32 index, const core::plane3df& plane, bool enable)
{
...
void COpenGLDriverOld::uploadClipPlane(u32 index)
{
...
void COpenGLDriverOld::enableClipPlane(u32 index, bool enable)
{
	if (index >= MaxUserClipPlanes)
		return;
	if (enable)
...
core::dimension2du COpenGLDriverOld::getMaxTextureSize() const
{
...
GLenum COpenGLDriverOld::primitiveTypeToGL(scene::E_PRIMITIVE_TYPE type) const
{
	switch (type)
	{
	case scene::EPT_POINTS:
...
GLenum COpenGLDriverOld::getGLBlend(E_BLEND_FACTOR factor) const
{
	GLenum r = 0;
	switch (factor)
	{
...
GLenum COpenGLDriverOld::getZBufferBits() const
{
	GLenum bits = 0;
...
bool COpenGLDriverOld::getColorFormatParameters(ECOLOR_FORMAT format, GLint& internalFormat, GLenum& pixelFormat,
...
COpenGLDriverOld::E_OPENGL_FIXED_PIPELINE_STATE COpenGLDriverOld::getFixedPipelineState() const
{
	return FixedPipelineState;
}

void COpenGLDriverOld::setFixedPipelineState(COpenGLDriverOld::E_OPENGL_FIXED_PIPELINE_STATE state)
{
	FixedPipelineState = state;
}

const SMaterial& COpenGLDriverOld::getCurrentMaterial() const
{
	return Material;
}

COpenGLCacheHandlerOld* COpenGLDriverOld::getCacheHandler() const
{
	return CacheHandler;
}
...
#if defined(_IRR_COMPILE_WITH_WINDOWS_DEVICE_) || defined(_IRR_COMPILE_WITH_X11_DEVICE_) || defined(_IRR_COMPILE_WITH_OSX_DEVICE_)
IVideoDriver* createOpenGLDriverOld(const SIrrlichtCreationParameters& params, io::IFileSystem* io, IContextManager* contextManager)
{
#ifdef _IRR_COMPILE_WITH_OPENGL_
	COpenGLDriverOld* ogl = new COpenGLDriverOld(params, io, contextManager);
...
{
#ifdef _IRR_COMPILE_WITH_OPENGL_
	return new COpenGLDriverOld(params, io, device);
#else
	return 0;
#endif //  _IRR_COMPILE_WITH_OPENGL_
}

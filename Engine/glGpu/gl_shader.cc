...
static void print_resource(std::ostream& os, const ShaderCreateInfo::Resource& res, bool auto_resource_location)
{
	...
	else if(1)//GLContext::explicit_location_support)
	{
...
std::string GLShader::geometry_interface_declare(const ShaderCreateInfo& info) const
{
	...
	for(const StageInterfaceInfo* iface : info.vertex_out_interfaces_)
	{
		bool has_matching_output_iface = true;//find_interface_by_name(info.geometry_out_interfaces_, iface->instance_name) != nullptr;
	...
	{
		bool has_matching_input_iface = true;// find_interface_by_name(info.vertex_out_interfaces_, iface->instance_name) != nullptr;
...
static void STR_CONCAT(char* tdst, size_t& len, char* suffix)
{
	char* dst = tdst + len;
	size_t slen = strlen(suffix);

	memcpy(dst, suffix, slen);
	len += slen;
	dst[len] = '\0';
}
...
static char* glsl_patch_default_get()
{
	...
	if(1)//GLContext::texture_gather_support)
	{
...
static char* glsl_patch_compute_get()
{
	...
	if(0)//if(GLContext::texture_cube_map_array_support)
	{
		STR_CONCAT(patch, slen, "#extension GL_ARB_texture_cube_map_array : enable\n");
		STR_CONCAT(patch, slen, "#define GPU_ARB_texture_cube_map_array\n");
	}
...
GLuint GLShader::create_shader_stage(GLenum gl_stage, std::string sources)
{
